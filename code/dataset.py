...
class AudioDataset(Dataset):
    def __init__(self, csv_file, transform=None, target_sample_rate=16000, max_length=None):
        """
        Args:
            csv_file (str): Path to CSV file containing 'path' and 'label' columns.
            transform (callable, optional): Optional transform to be applied on audio.
            target_sample_rate (int): Resample all audio to this sample rate.
            max_length (float/int): Optional max length in seconds to crop/pad audio.
        """
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            waveform = torch.zeros(1, self.sample_rate)  # fallback
            sr = self.sample_rate
        ...
        # --- Apply optional transform (e.g., log-mel spectrogram) ---
        if self.transform:
            try:
                waveform = self.transform(waveform)
            except Exception as e:
                print(f"Error applying transform on {audio_path}: {e}")
